...
This reference is paired with QAMap 0.5.0. Use a compatible local binary first;
...
2. Run QAMap from the repository root. Prefer an already installed local binary:
   ...
   ```sh
   npm exec --yes --registry=https://registry.npmjs.org --package=@ivorycanvas/qamap@0.5.0 -- qamap qa . --base <base> --head HEAD --format agent
   ```
...
3. Read `analysisScope` before interpreting package-relative evidence or commands.
   ...
   ```sh
   npm exec --yes --registry=https://registry.npmjs.org --package=@ivorycanvas/qamap@0.5.0 -- qamap qa <package-path> --workspace-root . --base <base> --head HEAD
   ```
...
5. Follow the `route.nextAction` contract:
   ...
   - `run-repository-command` — when policy permits repository code execution, prefer QAMap's bounded executor so analysis and execution share one receipt:
     ...
     ```sh
     npm exec --yes --registry=https://registry.npmjs.org --package=@ivorycanvas/qamap@0.5.0 -- qamap qa run . --base <base> --head HEAD --format agent
     ```
...
6. Only after a human or team accepts the scenario and automation adapter, create or preview executable coverage:
   ...
   ```sh
   npm exec --yes --registry=https://registry.npmjs.org --package=@ivorycanvas/qamap@0.5.0 -- qamap e2e draft . --base <base> --head HEAD --dry-run
   ```
...
```sh
npm exec --yes --registry=https://registry.npmjs.org --package=@ivorycanvas/qamap@0.5.0 -- qamap manifest init .
```
