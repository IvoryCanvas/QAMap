...
This reference is paired with QAMap 0.5.1. Use a compatible local binary first;
never retry a missing registry version or substitute `latest` without permission.

Read this only for explicitly requested deeper review, older binaries without `qa brief`, optional repository
command execution, automation drafts, or manifest repair. The main skill's consent
rules still apply. Do not run a second analysis just to interpret an already
completed brief or handoff.

## Saved JSON Handoff

`qamap qa report . --base <base> --head HEAD --handoff` returns the versioned
`qamap.qa.handoff` JSON envelope for tools that need structured evidence. When
`evidenceArchive.required` is true, `qamap qa read <file> --sha256 <hash> --bytes <n>`
returns checked pages; continue with `--offset <nextOffset>` only when the whole
archive is explicitly required. Paging a large archive into a model session costs
more than a brief; prefer `qa brief` for review. For save-only requests, omit
`--handoff`, return the report paths, and stop without reading them.
...
2. Run QAMap from the repository root. Prefer an already installed local binary:
   ...
   ```sh
   npm exec --yes --registry=https://registry.npmjs.org --package=@ivorycanvas/qamap@0.5.1 -- qamap qa . --base <base> --head HEAD --format agent
   ```
...
3. Read `analysisScope` before interpreting package-relative evidence or commands.
   ...
   ```sh
   npm exec --yes --registry=https://registry.npmjs.org --package=@ivorycanvas/qamap@0.5.1 -- qamap qa <package-path> --workspace-root . --base <base> --head HEAD
   ```
...
4. Read and verify intent before generating code. In agent format:
   ...
   - `execution`: check this first. Plain `qa` is `not-run`; `qa run` may return a bounded `passed`, `failed`, or `blocked` repository-validation receipt. If `performed` is true, do not execute the selected command again. For completed runs, inspect `execution.gitState`: a green command with `changed: true` still requires review of the bounded changed-path list.
...
5. Follow the `route.nextAction` contract:
   ...
   - `run-repository-command` — when policy permits repository code execution, prefer QAMap's bounded executor so analysis and execution share one receipt:
     ...
     ```sh
     npm exec --yes --registry=https://registry.npmjs.org --package=@ivorycanvas/qamap@0.5.1 -- qamap qa run . --base <base> --head HEAD --format agent
     ```
...
6. Only after a human or team accepts the scenario and automation adapter, create or preview executable coverage:
   ...
   ```sh
   npm exec --yes --registry=https://registry.npmjs.org --package=@ivorycanvas/qamap@0.5.1 -- qamap e2e draft . --base <base> --head HEAD --dry-run
   ```
...
```sh
npm exec --yes --registry=https://registry.npmjs.org --package=@ivorycanvas/qamap@0.5.1 -- qamap manifest init .
```
