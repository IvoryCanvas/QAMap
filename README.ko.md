...
`qamap qa brief`는 줄 번호가 붙은 diff, 변경 코드의 테스트와 호출 코드, QA 초점을
한 번의 제한된 응답으로 보여줍니다. `qamap qa report`는 결과를 로컬 파일로 저장하고
경로만 알려줍니다. [보고서만 저장하고 나중에 해석하기](docs/ko/agent-integration.md#결과를-읽지-않고-파일로만-저장하기)
...
JS/TS 분석 결과를 재사용하고, 패키지를 넘는 import와
