...
현재 브랜치는 **0.5.0-rc.1 출시 후보**를 준비 중이며 아직 배포하지 않았습니다.
`latest` 패키지와 플러그인 디렉터리는 별도로 배포됩니다.
[이번 버전의 범위와 남은 확인 항목](docs/releases/0.5.0.md)을 참고하세요.
...
개발 빌드의 [한 번에 검수 근거 받기](docs/ko/agent-handoff.md)도 확인할 수 있습니다.
...
개발 브랜치의 `qamap qa report`는 결과를 로컬 파일로 저장하고 경로만
알려줍니다. [보고서만 저장하고 나중에 해석하기](docs/ko/agent-integration.md#결과를-읽지-않고-파일로만-저장하기)
...
개발 브랜치에서는 JS/TS 분석 결과를 재사용하고, 패키지를 넘는 import와
export 연결에서 관련 테스트와 등록 지점의 후보를 찾습니다.
[분석 범위와 한계](docs/ko/repository-discovery.md)를 함께 표시하며,
제품 전체를 이해했다거나 실제 토큰 절감량을 측정했다는 뜻은 아닙니다.
