...
export function buildAgentQaSection(dlxCommand: string, reviewMode: AgentReviewMode = "ask"): string {
  return [
    ...
    "- Allocate at least 16,384 output tokens for the handoff when supported; confirm the JSON is untruncated. Interpret `summary` and `inlineReview` when present, otherwise `reviewEvidence`. inlineReview contains every record of the archive text as lossless tables: for each row, concatenate literal string parts and substitute numeric parts with that row's zero-based column. Inspect all rows, including exceptional values. It replaces the preview; do not read or expand the archive again. Cite original file/line evidence. Omitted paths/gaps in the text remain unknown; tests are not-run.",
    "- If `evidenceArchive.required` is true, use `qamap qa read <evidenceArchive.review.file> --sha256 <review.sha256> --bytes <review.bytes>` (archive fields for older receipts). Read one page per tool response, continuing with `--offset <nextOffset>` until null; never concatenate pages into one output. Each response is at most 16,384 bytes; allocate at least 8,192 output tokens when supported and confirm untruncated JSON. The reader verifies the full file each time without reanalysis. This is report review, not permission to scan source or run tests. If context or command limits prevent completion, report incomplete review, ask to narrow the change, and do not claim savings.",
    "- Resolve `excerptRef` within its own response or archive; `via` contains intermediate call evidence. Nonconsecutive line numbers indicate omitted context.",
    "- Do not add git/source searches, reread summary files, or rerun analysis. Report missing evidence as unknown and ask before expanding scope.",
