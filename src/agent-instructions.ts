...
export function buildAgentQaSection(dlxCommand: string, reviewMode: AgentReviewMode = "ask"): string {
  return [
    ...
    "- If `evidenceArchive.required` is true, read `evidenceArchive.review.file` (or `evidenceArchive.file` for older receipts) and verify its byte count and SHA-256 before concluding. The text view merges repeated source lines and paths without removing evidence. This generated report is part of review, not permission to scan source or run tests. If it exceeds host reading/context limits, report incomplete review and ask to narrow the change; do not claim savings.",
