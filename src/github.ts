...
const commentMarker = "<!-- qamap-pr-comment -->";

export async function runGitHubAction(rootInput: string, options: GitHubActionOptions = {}): Promise<GitHubActionResult> {
  const mode = resolveMode(options.mode);
  const reportFile = path.resolve(options.reportFile ?? "qamap-report.md");
  const commentFile = path.resolve(options.commentFile ?? "qamap-pr-comment.md");
  const testPlanFile = options.testPlan ? path.resolve(options.testPlanFile ?? "qamap-test-plan.md") : undefined;
  const evalFile = options.evaluation ? path.resolve(options.evalFile ?? "qamap-eval.md") : undefined;
  ...
  console.log(`QAMap ${mode} report: ${reportFile}`);
  console.log(`QAMap PR comment body: ${commentFile}`);
  if (testPlanFile) {
    console.log(`QAMap test plan: ${testPlanFile}`);
  }
  if (evalFile) {
    console.log(`QAMap eval: ${evalFile}`);
  }
...
function writeAnnotations(annotations: AnnotationInput[]): void {
  ...
  if (annotations.length > annotationLimit) {
    console.log(
      `::notice title=QAMap::${annotations.length - annotationLimit} additional QAMap findings were omitted from annotations.`,
    );
  }
}
