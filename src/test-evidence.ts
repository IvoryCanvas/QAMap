...
export async function collectTestSuiteInventory(root: string): Promise<TestSuiteInventory> {
  ...
  const files = projectFiles
    .filter((file) => isTestLikeFile(file.path) && !isGeneratedCodeWardDraft(file))
...
function isGeneratedCodeWardDraft(file: ProjectFile): boolean {
  return /\bGenerated by CodeWard\b/.test(file.text ?? "");
}
