...
| Bounded repository validation | `qa run executes the selected focused repository test and returns a bounded receipt`; `qa run does not embed raw command output in agent-facing receipts`; `qa run blocks product automation routes instead of executing an E2E draft`; `qa run terminates timed-out repository commands`; `qa run CLI preserves repository failure status and emits its receipt` | Explicit `qamap qa run` re-analyzes the change and executes only the exact selected existing repository-validation command. It reports passed, failed, blocked, or timed-out status with duration, byte counts, and output hashes; raw stdout and stderr remain outside JSON and agent receipts. Product automation routes, neutralized instruction text, and unsupported commands are never executed. |
