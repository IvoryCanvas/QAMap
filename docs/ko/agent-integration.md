...
로컬 0.5.0-rc.1 출시 후보에서는 [한 번의 호출로 요약과 코드 근거를 받는 방식](agent-handoff.md)도
준비하고 있습니다. 사용자의 동의 후 실행하고 LLM은 반환된 결과만 읽습니다.
별도 소스 검색은 하지 않으며, 근거가 부족하면 알리고 추가 검수를 요청할지
묻습니다. 기존 저장 전용 모드는 유지합니다. 이 기능은 0.4.17에는 없습니다.
[합성 사례 6쌍 비교](../report-only-validation.md#completion-wait-follow-up)에서는
사전에 정한 결함 발견 기준을 지키며 총 토큰 합계가 33.47% 줄었습니다.
큰 변경과 동적 모듈의 근거 누락은 남아 있으므로, 다른 작업에서도 같은
품질과 절감 효과를 유지한다고 보장하지 않습니다.

## 프로젝트의 검수 방식 저장하기

**로컬 출시 후보 전용 설정입니다. 아직 배포하지 않았으며 0.4.17에서는 지원하지 않습니다.**
