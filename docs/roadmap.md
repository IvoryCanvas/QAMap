...
- Preserve behavioral contracts declared by changed tests, including non-English test names, and route their exact files into repository validation without claiming execution. Node test, Vitest, Jest, and Playwright package scripts are narrowed only when their existing command shape is safe to interpret; custom shell pipelines keep the repository's full suite unchanged.
