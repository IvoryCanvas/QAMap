...
6. Read `readiness.basis` and `readiness.automationApplicable` before using the legacy `score` or `level`. For product behavior, they describe optional automation readiness, not a verdict on the PR. For analyzer, configuration, documentation, generated-artifact, or existing-test changes, `automationApplicable` is false and `verificationStatus` says whether an existing repository command is ready to run. Neither state means execution passed. `requiredBootstrap` contains non-runner repository context only; runner adoption lives under the opt-in `automation` object.
7. Use `flows[].verificationMode` before choosing an artifact: values such as `analysis-rule` or `existing-test-evidence` mean validating existing repository behavior, not inventing a product E2E. Then use `flows[].changedFiles`, `flows[].evidence`, and `flows[].reviewQuestion` to understand why the flow was selected. Use `steps`, `selectors`, and `successSignal` to write or review tests; `flows[].scenarioAutomation` is the compact selected-to-draft map and `runnable` says how much to trust the generated draft. Even emergency 4KB compaction prioritizes one located reasoning trace, one scenario source, the first flow's changed file, review question, success signal, one evidence gap, and one next command over exhaustive lists.
...
| `flows` | array | Affected user flows, most relevant first (capped). Each has `title`, `source`, backward-compatible `draft`, optional `runnable`, `entry`, and `verificationMode`, plus `changedFiles`, `reviewQuestion`, `successSignal`, `steps`, `selectors`, short `evidence` reasons, and compact `scenarioAutomation` entries (`id`, `decision`, `status`). `existingEvidence` exposes directly imported, same-stem, or owner-path-related tests; test-only changes use it for the changed tests themselves. Analyzer rules, configuration, docs, generated artifacts, and changed tests use `verificationMode`. |
