...
| `execution` | object | Receipt for this invocation. Plain `qa` returns `not-run` static mapping. `qa run` returns `passed`, `failed`, or `blocked` for one exact selected existing repository validation command. Completed receipts carry bounded metadata and hashes, not raw command output. |
...
```json
{"execution":{"status":"passed","performed":true,"scope":"repository-validation","command":"npm test","cwd":".","exitCode":0,"durationMs":842,"timedOut":false,"stdoutBytes":391,"stderrBytes":0,"stdoutSha256":"<64 hex characters>","stderrSha256":"<64 hex characters>"}}
```
